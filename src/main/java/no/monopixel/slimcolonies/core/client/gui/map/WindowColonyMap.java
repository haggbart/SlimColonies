...
import no.monopixel.slimcolonies.api.colony.requestsystem.requestable.IStackBasedTask;
                ...
                if (building.getColony().isCoordInColony(Minecraft.getInstance().level, player.blockPosition()))
                {
                    ...
                    if (playerImage == null)
                    {
                        final ResourceLocation resourceLocation =
                            OutOfJarResourceLocation.ofMinecraftSkin(Minecraft.getInstance(), Minecraft.getInstance().getUser().getGameProfile(), null);
        ...
        if (buildingDecorations == null)
        {
            ...
            tooltip.hoverPane(uiBuilding)
                ...
                .appendNL(Component.literal(
                    "Citizens: " + (buildingView instanceof LivingBuildingView home ? home.getResidents().size() : buildingView.getAllAssignedCitizens().size())));
        ...
        for (final ICitizenDataView data : colony.getCitizens().values())
        {
            ...
            if (citizen != null)
            {
                ...
                if (!data.getJob().isEmpty())
                {
                    ...
                    builder.newLine().append(Component.translatable("no.monopixel.slimcolonies.coremod.gui.citizen.job.label", LanguageHandler.format(data.getJob())));
                }
