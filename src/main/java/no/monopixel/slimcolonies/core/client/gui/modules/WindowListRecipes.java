...
import no.monopixel.slimcolonies.core.network.messages.server.colony.building.worker.ChangeRecipePriorityMessage;
import no.monopixel.slimcolonies.core.network.messages.server.colony.building.worker.ToggleRecipeMessage;
...
public class WindowListRecipes extends AbstractModuleWindow
{
        ...
        recipeList.setDataProvider(new ScrollingList.DataProvider()
        {
            ...
            public void updateElement(final int index, @NotNull final Pane rowPane)
            {
                ...
                if (module.isDisabled(recipe))
                {
                    ...
                    rowPane.findPaneOfTypeByID(BUTTON_TOGGLE, Button.class).setText(Component.translatable("no.monopixel.slimcolonies.coremod.gui.recipe.enable"));
                }
                else
                {
                    rowPane.findPaneOfTypeByID("gradient", Gradient.class).setVisible(false);
                    rowPane.findPaneOfTypeByID(BUTTON_TOGGLE, Button.class).setText(Component.translatable("no.monopixel.slimcolonies.coremod.gui.recipe.disable"));
                }
    ...
    public void onUpdate()
    {
        ...
        recipeStatus.setText(Component.translatable(TranslationConstants.RECIPE_STATUS, module.getRecipes().size()));
